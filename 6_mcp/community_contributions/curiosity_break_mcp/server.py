"""
Curiosity Break MCP — a tiny server for quick fun facts (no API keys).

Run modes:
  python server.py                  → stdio (default; Cursor local / Agents SDK)
  python server.py --streamable-http → HTTP on FASTMCP_HOST:FASTMCP_PORT (default 127.0.0.1:8000)
      Open http://127.0.0.1:8000/ in a browser for a human-readable page (MCP clients use /mcp).
  python server.py --sse            → SSE transport (older HTTP style)
...
async def browser_dashboard(request: Request) -> HTMLResponse:
    """Simple HTML UI — only active when you run with --streamable-http (or --sse). /mcp stays for machines."""
